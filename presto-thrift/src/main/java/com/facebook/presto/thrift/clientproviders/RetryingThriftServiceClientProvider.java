...
public class RetryingThriftServiceClientProvider
        implements ThriftServiceClientProvider
{
    private static final Logger log = Logger.get(RetryingThriftServiceClientProvider.class);
    private final ThriftServiceClientProvider original;
    private final RetryDriver retry;

    @Inject
    public RetryingThriftServiceClientProvider(@NonRetrying ThriftServiceClientProvider original)
    {
    ...
    {
        return new RetryingClient(original::anyHostClient, retry);
    }

    @Override
    public ThriftServiceClient selectedHostClient(List<HostAddress> hosts)
    {
        return new RetryingClient(() -> original.selectedHostClient(hosts), retry);
    }

    @NotThreadSafe
    private static final class RetryingClient
            implements ThriftServiceClient
    {
        private final Supplier<ThriftServiceClient> clientSupplier;
        private final RetryDriver retry;
        private ThriftServiceClient client;

        public RetryingClient(Supplier<ThriftServiceClient> clientSupplier, RetryDriver retry)
        {
            this.clientSupplier = requireNonNull(clientSupplier, "clientSupplier is null");
            this.retry = retry.onRetry(this::close);
        }

        private ThriftServiceClient getClient()
        {
            if (client != null) {
                return client;
            }
        ...
        {
            return retry.run("getTableLayouts", () -> getClient().getTableLayouts(session, schemaTableName, outputConstraint, desiredColumns));
        }
        ...
        {
            return retry.run("getSplits", () -> getClient().getSplits(session, layout, maxSplitCount, nextToken));
        }
