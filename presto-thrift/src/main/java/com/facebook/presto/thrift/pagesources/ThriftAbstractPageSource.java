...
public abstract class ThriftAbstractPageSource
        implements ConnectorPageSource
{
    ...
    public ThriftAbstractPageSource(List<ColumnHandle> columns, ThriftConfig config)
    {
        ...
        if (future != null) {
            if (!future.isDone()) {
                ...
                if (canGetMoreData(nextToken)) {
                    future = sendRequestForDataWithLatencyMeasurement();
                }
                else {
                    future = null;
                }
    ...
    private CompletableFuture<ThriftRowsBatch> sendRequestForDataWithLatencyMeasurement()
    {
        final long start = System.nanoTime();
        ListenableFuture<ThriftRowsBatch> rowsBatchFuture = sendRequestForData(nextToken, maxBytesPerResponse);
    ...
    public abstract ListenableFuture<ThriftRowsBatch> sendRequestForData(byte[] nextToken, long maxBytes);
