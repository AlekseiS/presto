...
import com.facebook.presto.connector.thrift.pagesources.ThriftIndexPageSource;
...
import static com.facebook.presto.connector.thrift.ThriftColumnHandle.columnNames;
        ...
        implements ConnectorIndex
{
    ...
    {
        PrestoThriftRowsBatch keys = convertKeys(recordSet, inputColumnNames);
        return new ThriftIndexPageSource(clientProvider, indexId, keys, outputColumns, config);
    }

    private static PrestoThriftRowsBatch convertKeys(RecordSet recordSet, List<String> columnNames)
    {
        ...
        List<PrestoThriftColumnData> columnsData = new ArrayList<>(numColumns);
        for (ColumnWriter writer : columnWriters) {
            columnsData.addAll(writer.getResult());
        }
        return new PrestoThriftRowsBatch(columnsData, totalRecords, null);
    }
}
