...
public final class PrestoThriftPage
{
    ...
    public static PrestoThriftPage fromRecordSet(RecordSet recordSet)
    {
        List<Type> types = recordSet.getColumnTypes();
        int numberOfColumns = types.size();
        int positions = totalRecords(recordSet);
        if (numberOfColumns == 0) {
            return new PrestoThriftPage(ImmutableList.of(), positions);
        }
        ...
        return new PrestoThriftPage(thriftBlocks, positions);
    }
